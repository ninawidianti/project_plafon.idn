...
const Daftarcafe2 = () => {
  ...
  const filteredCafes = cafes
    .filter((cafe) => cafe.kategori === "resto") // filter kategori resto dulu
    .filter((cafe) => {
  ...
  // Tambahkan helper di atas return/JSX
  const parseFotoCafe = (foto) => {
    if (!foto) return [];
    if (Array.isArray(foto)) return foto;
    try {
  ...
  return (
    <div className="container">
      ...
      <div className="daftar-title">
        <h1>Makan Serius di Tempat Santai</h1>
          ...
          filteredCafes.map((cafe) => {
            ...
            return (
              <div className="cafe-card" key={cafe.id} onClick={() => navigate(`/deskripsi/${cafe.id}`)} style={{ cursor: "pointer" }}>
                <img src={`http://localhost:5000/uploads/${fotoUtama}`} alt={cafe.name} />
                <h3>{cafe.name}</h3>
                <p>{cafe.alamat}</p>
